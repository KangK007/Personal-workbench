...
个人工作台 0.1 是一款面向科研学习与个人项目的本地优先 Flutter 应用。它把“快速收集、项目规划、安排今天、专注执行、知识沉淀、回顾、目标与行为追踪”放在一条可验证的工作流中，并融合了 SelfControl 的 Windows 自律限制能力。本轮界面采用清新绿色玻璃拟态体系：关键导航、页头和重点区域使用克制的半透明玻璃层，长列表与密集信息区保持高不透明度，以兼顾层级、可读性和滚动性能。
...
lib/
  ...
  core/theme/        浅色/深色主题、玻璃表面和语义颜色令牌
