...
视觉方向：采用“青绿雅韵新中式”，约 80% 现代软件 UI + 20% 东方古籍图鉴细节。亮色为暖古纸与玉石绿，暗色为黛青水墨，并以低饱和朱红、靛蓝、墨黑、鎏金作为传统色彩点题。不是传统古风网页，不要大面积书法，不要玻璃拟态，不要霓虹，不要荧光红和大面积金色。纸张纤维、淡墨、工笔枝叶、山石轮廓、祥云、回纹、缠枝纹和古铜节点只能在画布边缘、空状态、分隔线和专注留白以 2%-7% 透明度出现，不能放在文字或表单后面。
