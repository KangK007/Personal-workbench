...
> 本文档是历史实现记录和可选建议，不是项目级 UI 约束。后续界面可以采用新的颜色、字体、组件、布局、导航和动效方案，不需要保持本文档描述的视觉外观。
...
> **历史说明（2026-08-19）**：以下视觉方向、组件规范和响应式建议只保留为当时的实现记录；其中构建与测试命令仍可按当前工具链情况使用，视觉内容不构成后续实现要求。
