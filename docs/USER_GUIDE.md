...
“专注”页顶部同时提供“专注”和“自律”两个标签。进入“自律”后可管理限制时段、应用与网站名单、保护设置、拦截日志和 hosts 诊断；Android 端只编辑并同步规则，实际进程结束和 hosts 限制仅由 Windows 执行。

![专注页中的自律规则与保护设置](../docs/images/guide_focus_restriction.png)

图 8  自律能力合并到专注页，规则默认先预览再启用；SelfControl 导入入口位于页面右上角。
