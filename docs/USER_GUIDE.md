...
> **平台说明（2026-09-20）**：当前版本同时支持 Windows 桌面和 Android。Windows 提供托盘、通知、前台置前和系统级快速新增；Android 使用底部导航与「更多」菜单访问各模块，两端读取同一套记录模型。Android 不会结束 Windows 进程或修改 hosts。
...
Windows 桌面默认打开“今日”。左侧按心智域组织任务、项目、执行、记录、行为和系统入口。Android 使用“今日、任务、回顾、行为、设置”五项底部导航，其他页面从“更多”菜单进入。
...
桌面左侧有应用标题、个人别名、快速新增、全局搜索和按页面族分组的入口。导航栏可以用右上角的收起按钮缩窄；收起只改变宽度，不隐藏页面。点击左下角同步状态可以查看本地或云端连接提示。

![Windows 今日页和分组导航](../test/goldens/wide_shell_today_light.png)

图 1  Windows 宽屏首屏。左侧是按页面族组织的分组入口，右侧的“今日”包含下一步、时间安排和今日重点；状态线、图标和文字共同表达状态，不应只凭颜色判断。
...
| 行为 | 在习惯追踪与国策协议之间切换；国策协议包含国策树、国策库、轮次历史和分析 | 添加一个足够小的根节点 |
| 设置 | 逻辑日、提醒、托盘、检测、备份、同步和实验功能 | 先备份，再调整提醒和隐私开关 |

### 1.3 三个全局入口

- **快速新增**：打开一个轻量表单，先写一句话，再决定是否完善日期、项目、协议等字段。
- **全局搜索**：Windows 使用 `Ctrl+K`，可搜索任务、项目、笔记、回顾、目标和链接；在笔记页进入时默认筛选“仅笔记”。
...
2. 保存“每日收尾快照”作为可追溯记录。
3. 自动创建或打开当天的“日回顾”草稿，并切换到“回顾 → 日回顾”。
...
桌面侧栏提供“专注”和“自律”两个独立入口。自律页可管理限制时段、应用与网站名单、保护设置和拦截日志；开机启动、关闭窗口进入托盘以及 hosts 诊断统一放在主设置。Android 端只编辑并同步规则，实际进程结束和 hosts 限制仅由 Windows 执行。
...
桌面侧栏和 Android“更多”抽屉在“回顾”下提供 **日回顾 / 周回顾 / 月回顾** 三个子级。左右箭头切换上一期间或下一期间，“本期”返回当前逻辑日、当前周或当前月。
...
- 用“全部回顾 / 日回顾 / 周回顾 / 月回顾 / 历史年回顾”筛选。尚未迁移的旧日记会标记为“待迁移”，保存后写入规范日回顾，原记录仍保留。
...
## 9. 行为中心：习惯追踪与 RSIP 国策协议

### 9.1 国策协议：四个视图和八种节点

进入“行为”默认打开“习惯追踪”，可在页面顶部切换到“国策协议”。国策协议模式默认打开“国策树”。树的根在下、子节点在上；支持缩放、平移、分支折叠和类型筛选。首版不使用拖拽改变父级，避免误移动整棵子树。
...
“账号与同步”用于 Supabase 注册、登录、退出和手动同步。云同步默认关闭；离线时继续使用本地 SQLite。虚拟积分和专注押注账本只保留在本机，不进入 Supabase。回顾和笔记不是端到端加密，同步科研敏感内容前应确认数据管理要求。
...
3. 点击“完成每日收尾”。
4. 在自动打开的“回顾 → 日回顾”中补充心情、完成情况、阻塞和下一步，并点击保存。
...
- 目标迁移为项目，里程碑继续关联。
- 旧日记在回顾库中作为待迁移日回顾显示；保存后迁移为规范日回顾，原始记录保留。
- 旧 CTDP 嵌套组保留兼容显示。
- 成长、XP、签到和押注数据原样保留。
...
- 云同步前检查回顾和笔记是否包含不应离开本机的实验数据或个人信息。
