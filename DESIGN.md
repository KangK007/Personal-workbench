...
| 角色 | 浅色 | 深色 | 用途 |
| --- | --- | --- | --- |
| `canvas` | `#F2F7F3` | `#0F1712` | 清新浅绿页面背景 |
| `surface` | `#FBFDFB` | `#16231B` | 稳定阅读面 |
| `ink` | `#1B2B20` | `#E8F2EA` | 主文字 |
| `route` | `#2F7D57` | `#76C893` | 当前路径、主操作与焦点 |
...
- 页名使用文楷 Medium `500`；章节、对话框和编辑器标题使用 IBM Plex Sans SC SemiBold `600`；正文、说明和列表内容使用 Regular `400`。
- 字体文件位于 `assets/fonts/`，由 Flutter 与 `design_preview/` 离线加载；许可证分别保存为 `OFL-LXGW-WenKai-GB.txt` 和 `OFL-IBM-Plex.txt`。
- 字间距统一为 `0`，不随视口缩放字号。

## 形状、密度与层级

- 面板圆角 `12px`，控件 `8px`，对话框和底部弹层 `16px`；短状态标签使用完整胶囊圆角。
- 桌面内容行最小高度 `44px`，移动端内容行最小高度 `52px`，交互触控区域至少 `48dp`。
- 间距使用 `4 / 8 / 12 / 16 / 24 / 32`。
- 页面分区保持无框；卡片只用于重复记录、弹窗和确有边界的工具，不在卡片内嵌套卡片。
- 自律与设置列表使用 36px 浅色图标底、18px 描边图标和至少 16px 面板内边距，左右操作区共享同一对齐边界。
- 普通面板使用中性 `1px` 分隔线，raised 浮层使用单一柔和阴影建立层次；不在同一层同时堆叠宽阴影与描边。状态强调使用左缘标记、航迹节点或图标，不制造装饰性表面。
